  ...
  (gltf) => {
    ...
    shipModel.traverse((child) => {
      if (child.isMesh) {
        ...
        if (child.material) {
          child.material.roughness = 0.35; child.material.metalness = 0.65;
...
function animate() {
  ...
  if (shipModel) {
    const heatRatio = spd / MAX_SPEED; // 0 a 1
    shipModel.traverse((child) => {
      if (child.isMesh && child.material) {
        const c = child.material.color;
        // Detecta naranja: R alto, G medio, B casi 0
        if (c && c.r > 0.5 && c.g > 0.1 && c.g < 0.6 && c.b < 0.15) {
          if (!child.material.emissive) child.material.emissive = new THREE.Color();
          // Frío = negro, tibio = naranja tenue, caliente = naranja brillante, máximo = blanco naranja
          child.material.emissive.setRGB(
            heatRatio * 1.0,           // R sube rápido
            heatRatio * 0.35,          // G sube lento (naranja → blanco)
            heatRatio * 0.05           // B casi nada
          );
          child.material.emissiveIntensity = heatRatio * 2.5;
        }
      }
    });
  }
